...
function getEventDetails(events) {
    ...
    events.forEach(event => {
        ...
        begin = event.begin.split(' ');

        console.log(begin);
        addEvent(event.title, event.date, event.time, event.id, event.test)
        // console.log(event);
    });
}

//same as generate month except we add events to the month
function addEvent(title, date, time, id, test) {
        ...
        for (var d in days) {
            if (currentMonth.month == days[d].getMonth()) {
                ...
                if (eventYear == days[d].getFullYear() && eventMonth == days[d].getMonth() + 1 && eventDay == days[d].getDate()) {
                    ...
                    eventTime.appendChild(document.createTextNode(time + '-' + test));
