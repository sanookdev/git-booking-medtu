<?
    ...
    $myRes = array();
    $sql = "SELECT res.*,room.name ,room.detail,room.building_id, room.class_no,u.username,b.building_name FROM reservation AS res
                JOIN rooms AS room ON res.room_id = room.id
                    JOIN user AS u ON res.member_id = u.id_card
                        JOIN building AS b ON b.id = room.building_id
                            ORDER BY res.status, res.create_date  DESC";
    ...
    while($row = $result->fetch_assoc()){
        $data[$i] = $row;
        $myRes[$i]['id'] = $row['id'];
        $myRes[$i]['building_name'] = $row['building_name'];
        $myRes[$i]['topic'] = $row['topic'];
        $myRes[$i]['room_name'] = $row['name'];
        $myRes[$i]['class_no'] = $row['class_no'];
                ...
                <table class="table table-bordered shadow" id="data-table" style="width:100%">
                    ...
                    <tbody class="bg-light text-center">
                        ...
                        <tr>
                            <td><?= $i + 1; ?></td>
                            <td><?= $myRes[$i]['topic']; ?> </td>
                            <td><?= $myRes[$i]['room_name'].' / '.$myRes[$i]['building_name'].' ('.$myRes[$i]['class_no'].')'; ?>
                            </td>
